...
一个没有学过 Haskell 的 Scheme 程序员，最容易犯的一个错误就是，把除 `#f`（Scheme 的逻辑“假值”） 以外的任何值都作为 `#t`（Scheme 的逻辑“真值”）。很多人认为这是 Scheme 的一个“特性”，可是殊不知这其实是 Scheme 的极少数缺点之一。如果你了解 Lisp 的历史，就会发现在最早的时候，Lisp 把 `nil`（空链表）这个值作为 `False` 来使用，而把 `nil` 意外的其它值都当成 `True`。这带来了逻辑思维的混乱。
